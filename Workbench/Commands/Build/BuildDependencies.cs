...
public interface BuildDependency
{
    ...
    public Task InstallAsync(Executor exec, Vfs vfs, Dir cwd, BuildEnvironment env, Log print, BuildData data);
...
internal class DependencySdl2 : BuildDependency
{
    ...
    public async Task InstallAsync(Executor exec, Vfs vfs, Dir cwd, BuildEnvironment env, Log print, BuildData data)
    {
        ...
        if (false == root_folder.GetFile("INSTALL.txt").Exists(vfs))
        {
            ...
            Core.MoveFiles(vfs, print, root_folder.GetDir(folder_name), root_folder);
        }
        else
        {
        ...
        if (false == build_folder.GetFile("SDL2.sln").Exists(vfs))
        {
            ...
            await project.ConfigureAsync(exec, vfs, cwd, print);
            await project.BuildAsync(exec, vfs, cwd, print, Shared.CMake.Config.Release);
        }
        else
        {
...
internal class DependencyAssimp : BuildDependency
{
    ...
    public async Task InstallAsync(Executor exec, Vfs vfs, Dir cwd, BuildEnvironment env, Log print, BuildData data)
    {
        ...
        if (false == dependency_folder.Exists(vfs))
        {
            ...
            await project.InstallAsync(exec, vfs, cwd, print, Shared.CMake.Config.Release);
        }
        else
        {
...
internal class DependencyWxWidgets : BuildDependency
{
    ...
    public async Task InstallAsync(Executor exec, Vfs vfs, Dir cwd, BuildEnvironment env, Log print, BuildData data)
    {
        ...
        var build_rel = false == GetLibraryFolder().GetFile("wxzlib.lib").Exists(vfs);

        if (build_dbg || build_rel)
        {
            var project = await ConfigProjectAsync(exec, vfs, cwd, print, root_folder, build_folder, generator);
            if (build_dbg)
            {
                print.Info("building debug wxWidgets");
                await project.BuildAsync(exec, vfs, cwd, print, Shared.CMake.Config.Debug);
            }
    ...
    private static async Task<CMakeProject> ConfigProjectAsync(Executor exec, Vfs vfs, Dir cwd, Log print, Dir root, Dir build, Generator generator)
    {
