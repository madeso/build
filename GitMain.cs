...
namespace Workbench
{
        ...
        internal sealed class Status : Command<Status.Arg>
        {
            ...
            public override int Execute([NotNull] CommandContext context, [NotNull] Arg settings)
            {
                ...
                foreach(var line in r)
                {
                    ...
                    switch (line.Status)
                    {
                        case Workbench.GitStatus.Unknown:
                            AnsiConsole.MarkupLine($"Unknown [green]{line.Path}[/] ([blue]{status}[/]).");
                            break;
                        case Workbench.GitStatus.Modified:
    ...
    internal class GitMain
    {
        internal static void Configure(IConfigurator config, string name)
        {
            config.AddBranch(name, git =>
            {
                git.AddCommand<CmakeCommands.Status>("status");
                git.AddCommand<CmakeCommands.RemoveUnknown>("remove-unknown");
            });
        }
    }
}
